...
# Production
#*/5 * * * * /usr/bin/bash /opt/arise/client/arise_snapshot.sh  -b /opt/arise/backup -d 2 -g > /dev/null 2>&1

# Debug
#*/2 * * * * /usr/bin/bash /opt/arise/client/arise_snapshot.sh  -b /opt/arise/backup -g  > /opt/arise/client/logs/snapshot_cron."`date +\%Y\%m\%d_\%H\%M\%S`".log 2>&1
...
ARISE_CONFIG="config.json"
PM2_CONFIG="$(pwd)/etc/pm2-snapshot.json"
SOURCE_DB_NAME="$(grep "database" "$ARISE_CONFIG" | cut -f 4 -d '"')"
...
if [ ! -f "$LOCK_FILE" ]; then
  ...
  if [ "$( stat --format=%Y "$LOG_LOCATION" )" -le $(( $(date +%s) - ( STALL_THRESHOLD_PREVIOUS * 60 ) )) ]; then
    ...
    bash arise.sh stop_node -c "$SNAPSHOT_CONFIG"
...
bash arise.sh stop_node -p "$PM2_CONFIG" -c "$SNAPSHOT_CONFIG" &> /dev/null
...
bash arise.sh start -p "$PM2_CONFIG"
...
until tail -n10 "$LOG_LOCATION" | (grep -q "Snapshot finished"); do
  ...
  if [ "$( stat --format=%Y "$LOG_LOCATION" )" -le $(( $(date +%s) - ( STALL_THRESHOLD_CURRENT * 60 ) )) ]; then
    ...
    bash arise.sh stop_node -p "$PM2_CONFIG" &> /dev/null
    dropdb --if-exists "$TARGET_DB_NAME" &> /dev/null
    rm -f "$LOCK_FILE" &> /dev/null
    exit 1
  fi
...
HEIGHT="$(psql -d arise_snapshot -t -c 'select height from blocks order by height desc limit 1;' | xargs)"
