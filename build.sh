...
echo "Building arise..."
echo "--------------------------------------------------------------------------"
if [ ! -f "$ARISE_FILE" ]; then
  exec_cmd "wget $ARISE_URL -O $ARISE_FILE"
fi
if [ ! -d "$BUILD_NAME/node_modules" ]; then
  ...
  exec_cmd "npm install --production $ARISE_CONFIG"
...
echo "Building arise-node..."
echo "--------------------------------------------------------------------------"
if [ ! -f "$ARISE_NODE_FILE" ]; then
  exec_cmd "wget $ARISE_NODE_URL -O $ARISE_NODE_FILE"
fi
if [ ! -f "$ARISE_NODE_DIR/$ARISE_NODE_OUT" ]; then
  exec_cmd "rm -rf $ARISE_NODE_DIR"
  exec_cmd "tar -zxvf $ARISE_NODE_FILE"
  cd "$ARISE_NODE_DIR" || exit 2
  apply_patches "node"
  exec_cmd "./configure --without-npm $ARISE_NODE_CONFIG"
  exec_cmd "make --jobs=$JOBS"
  cd ../ || exit 2
fi
exec_cmd "mkdir -p $BUILD_NAME/nodejs"
exec_cmd "cp -f $ARISE_NODE_DIR/$ARISE_NODE_OUT $BUILD_NAME/nodejs/"
...
# Create arise-node-$OS-$ARCH.tar.gz
cd "$NOVER_BUILD_NAME" || exit 2
exec_cmd "GZIP=-6 tar -czf ../../release/arise-node-$OS-$ARCH.tar.gz nodejs"
cd ../ || exit 2

# Create arise-source.tar.gz
exec_cmd "mv -f $VERSION arise-source"
exec_cmd "GZIP=-6 tar -czf ../release/arise-source.tar.gz arise-source"
...
exec_cmd "$SHA_CMD arise-node-$OS-$ARCH.tar.gz > arise-node-$OS-$ARCH.tar.gz.SHA256"
exec_cmd "$SHA_CMD arise-source.tar.gz > arise-source.tar.gz.SHA256"
cd ../src || exit 2

echo "Cleaning up..."
echo "--------------------------------------------------------------------------"
exec_cmd "rm -rf $BUILD_NAME $NOVER_BUILD_NAME arise-source"
cd ../ || exit 2
